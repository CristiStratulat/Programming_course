...
i+j=n-1 (when we start counting from 0). Hence the elements under the 
main diagonal follow the rule that  i+j>=n. So we loop i from 1 to n
(we loop through every row) and we loop j from n-i to n (logical proof)
(columns that are under the secondary diagonal) 
*/
void under_sec_matrix(int a[][30], int n)
{
    int i,j;
